...
$sql = "SELECT * FROM reviews_monthly_personal_productivity WHERE user_id = '".mysqli_real_escape_string($conn, $_SESSION['viewing_client_id'])."' ORDER BY input_id DESC";
$result = mysqli_query($conn, $sql);

if(mysqli_num_rows($result) == 0){

	echo "<div class ='prompt-font' style='margin-top:20vh;'>Sorry you don't have any reviews currently completed<br>You may start to complete this at the <span style='color:red;'>end of this month</span>.</div>";
}
else{
//	require $_SERVER['DOCUMENT_ROOT']."/components/layouts/standard_page/spacings/start_of_page_top_of_centering_divs.php";
	...
	$sql = "SELECT * FROM work_diary WHERE user_id = '".mysqli_real_escape_string($conn, $_SESSION['user_id'])."' ORDER BY input_id DESC";
	$result = mysqli_query($conn, $sql);
...
//		echo " of ";
		echo date("F", date('n',strtotime($row['local_timestamp_on_entry'])) - 1);
		echo " ";	
...
echo "<div style='height:20px;'></div></div>";
