...
for($k = 0; $k <= count($ordered_results_array['database']) - 1; $k ++){		//need to specify ['database'] to ensure we just get out the number of results
	...
	if($initial_results_array['database'][$k] != 'issues'){
		$string_to_echo .= "[".$ordered_results_array[x_coordinate][$k].",".create_go_to_link($database,$item_id).",".$ordered_results_array[y_coordinate][$k].", createCustomHTMLContent($title),		null, 		null,		null],";	
...
while($row = mysqli_fetch_array($result)){
	...
	if($alternator == 'even'){
		for($i = 0; $i <= count($ordered_results_array['database']) - 1; $i ++){	
				if(	$row['item_a_database'] == $ordered_results_array['database'][$i] &&
					...
					$string_to_echo .= "	
				 	...
				 	[".$ordered_results_array[x_coordinate][$i].", 	".create_go_to_link($database,$item_id).", null, '',	".$ordered_results_array[x_coordinate][$i].", 	null,	null], 	 ";
				}
	  	...
	  	if($relationship_number <> mysqli_num_rows($result)){
	  		$string_to_echo .= "	[null, 			/map/links/,	null, 		'',							null, 		null,		null] ";
		}
		$relationship_number ++;	
	}
