...
//drawing lines can only be done between two points right next to each other
          [2,  		135, 		135, null,		'point { size: 18;}','html name for this tooltip - 2,135'],
          [250,  	250, 		250, null,		'point { size: 18; shape-type: star; fill-color: #a52714;}','html name for this tooltip'],
          [3,  		139, 		null, null,		'point { size: 18;}','html name for this tooltip'],
          [4,  		136, 		null,null, 		'point { size: 18;}','html name for this tooltip'],
          
          [250,  	250,		null,250, 		'point { size: 18; shape-type: star; fill-color: #a52714;}','html name for this tooltip']   ,           
          [300,  	300,		null,300, 		'point { size: 18;shape-type: circle;fill-color: #cecece;}','html name for this tooltip']          
        ]);
...
//https://stackoverflow.com/questions/38227335/how-to-make-line-clickable-on-google-line-chart
          ...
          if($nodes_count <= 2){echo ",         series: {".($link_number).": {type: 'line',color: 'yellow',lineWidth: 30  }}"; $link_number ++;}
          if($nodes_count <= 3){echo ",         series: {".($link_number).": {type: 'line',color: 'grey',lineWidth: 20  }}"; $link_number ++;}
          if($nodes_count >= 4){echo ",         series: {".($link_number).": {type: 'line'}}"; $link_number ++;}
          if($nodes_count >= 5){echo ",         series: {".($link_number).": {type: 'line'}}"; $link_number ++;}
          if($nodes_count >= 6){echo ",         series: {".($link_number).": {type: 'line'}}"; $link_number ++;}
          if($nodes_count >= 7){echo ",         series: {".($link_number).": {type: 'line'}}"; $link_number ++;}
          if($nodes_count >= 8){echo ",         series: {".($link_number).": {type: 'line'}}"; $link_number ++;}
          if($nodes_count >= 9){echo ",         series: {".($link_number).": {type: 'line'}}"; $link_number ++;}
          if($nodes_count >= 10){echo ",         series: {".($link_number).": {type: 'line'}}"; $link_number ++;}
