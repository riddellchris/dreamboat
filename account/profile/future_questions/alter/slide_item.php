...
$sql = "SELECT * FROM issues WHERE user_id ='".mysqli_real_escape_string($conn, $_SESSION['viewing_client_id'])."' AND item_id = '".$item_id."'";
...
$old_rank = $row['rank'];
$new_rank = $row['rank'] - 1;
 $sql = "SELECT * FROM issues WHERE  user_id ='".mysqli_real_escape_string($conn, $_SESSION['viewing_client_id'])."' AND rank = '".$new_rank."'";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
