...
<?php
	...
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		...
		if($all_null == 'yes'){
			$what_is_productivity[$month_combo_reverse[6]] = 0;
		} 
	}
	...
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		...
		if($all_null == 'yes'){
			$what_is_productivity_target[$month_combo[6]] = 0;
		};	
	};
	...
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		...
		for($i=0; $i<$month_to_loop; $i++){
			...
			if($all_null == 'yes'){
				$productivity_target[$month_combo[6]] = 0;
			}
		}
	}
