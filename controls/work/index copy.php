...
while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
	...
	for($i=0;$i<$month_to_loop;$i++){
		if($row[$month_combo[$i]] != 0){
			...
			// REVIEW: set to null or 0 ? because in productivity it's set to 0
			$income[$month_combo[$i]] = 'null';
		}
	}
}
	...
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		...
		for($i=0;$i<$month_to_loop;$i++){
			if($row[$month_combo[$i]] != 0){
				...
				$income_target[$month_combo[$i]] = 'null';
			}
		}
	}
	// Get Hours data
	...
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		...
		for($i=0;$i<$month_to_loop;$i++){
			if($row[$month_combo[$i]] != 0){
				...
				$hours_worked[$month_combo[$i]] = 'null';
			}
		}
	}
	...
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		...
		for($i=0;$i<$month_to_loop;$i++){
			if($row[$month_combo[$i]] != 0){
				...
				$hours_worked_target[$month_combo[$i]] = 'null';
			}
		}
	}
