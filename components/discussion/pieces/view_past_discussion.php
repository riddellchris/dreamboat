...
if($_SESSION['dreamboat_crew'] == 'yes'){$other_parties_id = $_SESSION['viewing_client_id'];}
	...
	if(	$_GET['secondary_folder'] 	== 'item' OR
		$_GET['tertiary_folder'] 	== 'item' OR
		$_GET['quarternary_folder']	== 'item'){
			$sql .= "   discussion.related_id = '".$_GET['item_id'] ."'";	
			$and_required = 'yes';  		  
		}		
		
		if($and_required == 'yes'){ $sql .= " AND ";$and_required = 'no';}
	...
	if( (	($_GET['primary_folder'] == 'management' && $_GET['secondary_folder'] == 'clients') OR
		...
		if ($_GET['primary_folder'] == 'admin' && $_GET['secondary_folder'] == 'technology'){
			...
			$sql .= "	discussion.primary_folder = 'admin' 		AND 
					...
					discussion.tertiary_folder = '".$_GET['tertiary_folder']."' AND
					discussion.quarternary_folder = '".$_GET['quarternary_folder']."' AND
					discussion.related_id = '".$_GET['item_id']."' 
				";

		}
		else{		
			$sql .= "discussion.primary_folder = '".$_GET['primary_folder']."'";
	
			if($_GET['tertiary_folder'] != 'item' AND
				$_GET['quarternary_folder'] != 'item'){
				$sql .= "  AND discussion.secondary_folder 	= 	'".$_GET['secondary_folder']."'";
				}
			...
			if(	$_GET['primary_folder'] != 'improvements' && 
				...
				$sql .= "  AND discussion.secondary_folder 	= 	'".$_GET['secondary_folder']."'
					...
					AND discussion.tertiary_folder 	= 	'".$_GET['tertiary_folder']."'";
			}
		}
	$and_required = 'yes';  
	}
	...
	$sql .= "   (
		  	(discussion.user_id = '".$_SESSION['user_id'] ."' 	AND	discussion.to_user_id = '".$other_parties_id."'   )
		  	OR
		  	(discussion.user_id = '".$other_parties_id."'		AND	discussion.to_user_id = '".$_SESSION['user_id'] ."'   )	  	
		  	)
	...
	.discussion_container{		
		...
		while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
			...
			<div class='discussion_container'";
				if($row['responding_to_prepared_prompt'] == 'yes'){echo " style='background-color: #f5f5f5;margin-bottom:12px;margin-top:12px;padding:10px;' title='The more of these questions that you respond to the better the system will help'";}
				if($row['private_pilot_advice'] == 'yes'){echo " style='background-color: #ffc4c4;margin-bottom:12px;margin-top:12px;padding:10px;' title='Private advice logged by a Pilot - for Pilot&#39;s eyes only.'";}
				echo ">";

				if(	$row['responding_to_prepared_prompt'] == 'yes'	OR
					$row['private_pilot_advice'] == 'yes'
					){
						echo "<span style='color: #281e96;text-align:left;'>";
						if($row['responding_to_prepared_prompt'] == 'yes'){echo $row['prompt_answered'];}
						if($row['private_pilot_advice'] 	 == 'yes'){echo ucwords($row['private_pilot_advice_category'])." based Pilot advice";}						
						echo "</span>";
				...
				if($row['system_notification'] == 'yes'){
					...
					if($comment_number < 9){
						if($row['user_id'] == $_SESSION['user_id']){$color = $me[$comment_number];}
						else{$color = $you[$comment_number];}	
					}
					else{
						if($row['user_id'] == $_SESSION['user_id']){$color = $me[8];}
						else{$color = $you[8];}						
					}
					echo "color: ".$color.";";
					echo "'>";
					...
					if($comment_number < 9){
						if($row['user_id'] == $_SESSION['user_id']){$color = $me_time_color[$comment_number];}
						else{$color = $you_time_color[$comment_number];}	
					}
					else{
						if($row['user_id'] == $_SESSION['user_id']){$color = $me_time_color[8];}
						else{$color = $you_time_color[8];}						
					}
					echo "color: ".$color.";";
					
					
					echo "float:right;font-size:14px;'>";	
					if($row['user_id'] == $_SESSION['user_id']){echo "you said ";}
					else{echo $row['first_name']." ".$row['second_name']." said ";}
