...
$sql = "SELECT *
	FROM user_services_active_per_user
	INNER JOIN user_email_settings ON user_services_active_per_user.user_id = user_email_settings.user_id
	WHERE user_services_active_per_user.progress_plus = 'yes' 
	AND user_email_settings.progress_plus = 'on'";
...
for($i=0; $i<$number_to_end_at; $i++){
	...
	$sql = "SELECT * FROM progress_plus_time_data WHERE user_id = '".$user_id."'";
	$result = mysqli_query($conn, $sql);
...
//echo "<br><br>";
	...
	echo 	$SecondsDifference/60;
		if($SecondsDifference > -1800 AND $SecondsDifference < 1800){
			...
			require $_SERVER['DOCUMENT_ROOT']."/back_of_house/database/connection.php";
			$sql = "INSERT INTO progress_plus_security (user_id, security_key, end_of_what) VALUES ('".$user_id."','".$security_key."','week')";
			
			$result = mysqli_query($conn, $sql);			
						
			$url		= "https://dreamboat.com.au/progress_plus/incoming.php?user_id=".$user_id."&security_key=".$security_key;
			require $_SERVER['DOCUMENT_ROOT']."/back_of_house/emails/progress_plus/end_of_week.php";
		}
	}
}
