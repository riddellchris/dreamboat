...
echo $sql = "SELECT * FROM user_email_settings 
 	 INNER JOIN user_account_details 
 	 ON user_email_settings.user_id = user_account_details.user_id 
 	 WHERE 	(user_email_settings.progress_plus_emails = 'on' 
	 	 AND user_account_details.status = 'active')";
...
//	echo "<br>";
		...
		if($SecondsDifference > -1800 AND $SecondsDifference < 1800){
			...
			$url		= "https://dreamboat.com.au/progress_plus/incoming.php?user_id=".$_SESSION['user_id']."&security_key=".$security_key;
			require $_SERVER['DOCUMENT_ROOT']."/back_of_house/emails/progress_plus/end_of_day.php";
			
			//set the icon to red
			$sql = "UPDATE user_alert_system
				SET top_nav_tracker = 'on' 
