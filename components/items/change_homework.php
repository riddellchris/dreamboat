...
$sql = "SELECT category FROM upgrades WHERE item_id = '".mysqli_real_escape_string($conn, $_GET['item_id'])."'";

$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_array($result, MYSQLI_ASSOC);

//redirect to where we actually need to go
header("Location: /upgrades/".$row['category']."/item/display.php?item_id=".$_GET['item_id']);
